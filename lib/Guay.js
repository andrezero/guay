...
function Guay (watcher, config, logger) {
    ...
    function renderNode(tree, node) {
        ...
        try {
            ...
            logger.warn(err, 'Error parsing template "' + template.filename + '" for node "' + node.filename + '"');
        }
    }
    ...
    function outputNode(node) {
        ...
        return fsExtra.ensureDirAsync(path.dirname(destination)).then(function () {
            ...
            logger.warn(err, 'Error saving file "' + destination + '"');
            return q.reject(err);
        });
    }
    ...
    function run(done) {
        let start = new Date();
        logger.info('Guay::run()');

        let tree = scan.paths(config.pages.paths);
        logger.debug({tree: tree.asString()}, 'Guay::run() files');

        let promise = tree.read();

        promise = promise.then(function () {
            logger.debug('Guay::run() process');
            return tree.process(processors);
        });

        promise = promise.then(function () {
            logger.debug('Guay::run() render');
        ...
        promise.then(function () {
            ...
            logger.info({ellapsed}, 'Guay::run() done');
            done();
        }).catch(function (err) {
            logger.fatal(err, 'Guay::run() error');
            process.exit();
        });
    }
